...
def optimize_random_start(upper_bounds):
    command_line=['nice','python','infer_between_cumulative.py',tract_filename, min_tract_length]
...
def randomize_and_optimize(upper_bounds, x0):
    ...
    command_line=['nice','python','infer_between_cumulative_specify_start.py',tract_filename, min_tract_length]
    for i in range(len(upper_bounds)):
        command_line.append(str(upper_bounds[i]))
    for i in range(len(x0)):
...
def choose_next_params(upper_bounds, x0):
    ...
    if hitbounds:
        ...
        prob_coal=[1-exp(-x0[1]/x0[num_size_changes])]
        margin=1-prob_coal[0]
        index_max_prob=0
        for i in range(1,num_size_changes-1):
            prob_no_coal=exp(-x0[i+1]/x0[num_size_changes+i])
...
def parse_history(history):
    output='Population sizes (divergence time to past) :'
    for i in range(len(history)/2, len(history)-1):
        output+=str(10000*history[i])+', '
    output+=str(10000*history[-1])+'\n'
    output+='Divergence time: '+str(generation_time*20*history[0])+'\n'
...
history=optimize_random_start([1.0])
upper_bounds=[history[0]]
